    ...
    if (isAbort) {
      icon = (
        <Tooltip title={t('tool.intervention.toolAbort')}>
          <Icon color={'var(--ant-color-text-tertiary)'} icon={PauseIcon} />
        </Tooltip>
      );
    } else if (isReject) {
      icon = (
        <Tooltip title={t('tool.intervention.toolRejected')}>
          <Icon color={'var(--ant-color-text-tertiary)'} icon={Ban} />
        </Tooltip>
      );
    } else if (hasError) {
      icon = <Icon color={'var(--ant-color-error)'} icon={X} />;
    } else if (isPending) {
      icon = <Icon color={'var(--ant-color-info)'} icon={HandIcon} />;
    } else if (hasSuccessResult && !hasError && successVariant === 'warning') {
      icon = <Icon color={'var(--ant-color-warning)'} icon={AlertTriangle} />;
    } else if (hasResult || isToolComplete) {
      icon = <Icon color={'var(--ant-color-success)'} icon={Check} />;
    } else {
